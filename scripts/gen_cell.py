...
n_bodies = len(meta["tube_centers"]) + 5 + 1 + 2   # 관 + 공기5 + 핀 + 케이싱2
...
jl = exporters.cell_label_journal(p, mesh_in="cell.msh.h5",
                                  ...
                                  face_seeds=seeds)
jb = exporters.cell_bsep_journal(p, mesh_in="cell.msh.h5")
...
(OUT / "sep_solver.py").write_text(jb, encoding="utf-8")

g = meta["geometry"]
print("[생성] examples/cell")
print(f"  바디 {n_bodies}  ·  전열면적 {area * 1e6:.1f} mm2")
print(f"  도메인 x {g['Lx']:.0f} · y {g['Ly']:.2f} · z {g['Lz']:.3f} mm")
print(f"  슬래브 {g.get('slab_mm', 0):.1f} mm · 케이싱 {g.get('cap_mm', 0):.1f} mm")
print(f"  입구면 {g['Ly'] * g['Lz']:.2f} mm2  (슬래브 유일 자유면)")
